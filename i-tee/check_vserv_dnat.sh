...
VSERV () {
	while true
	do
	bash /root/e-lab-check-scripts/check_objective/check_dnat_isp.sh &&
	bash /root/e-lab-check-scripts/check_objective/check_dnat_guest.sh &&
	bash /root/e-lab-check-scripts/check_objective/check_dnat_workstation.sh
	if [ $? -eq 0 ]; then
